...
class SelectableAttribute extends Attribute
{
    ...
    protected function init()
    {
        add_action('init', function () {
            ...
            add_filter( 'post_updated_messages',[$this, 'post_save_message'] );
     ...
     * @since  0.1.0
     */
    public final function post_save_message(array $messages) : array {

        $error = isset($_GET[CC_MYWC_PLUGIN_SLUG . '_admin_attr_save_error']) ? $_GET[CC_MYWC_PLUGIN_SLUG . '_admin_attr_save_error'] : false;
        if($error !== false && $error === "no_cat_tag_passed"){
            $error_message = esc_html__('Please, at least select one product category or product tag.', 'cupcode-mywc');
            $messages['post'][1] = $error_message;
            $messages['post'][4] = $error_message;
            $messages['post'][6] = $error_message;
            $messages['post'][7] = $error_message;
            $messages['post'][9] = $error_message;
            $messages['post'][10] = $error_message;
        }
        return $messages;
